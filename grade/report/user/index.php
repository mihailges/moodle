...
if (has_capability('moodle/grade:viewall', $context)) {
    ...
    if (is_null($userid)) {
        ...
        print_grade_page_head($courseid, 'report', 'user', ' ', false, null, true, null, null, null, $actionbar);
       ...
       // groups_print_course_menu($course, $gpr->get_return_url('index.php?id=' . $courseid, ['userid' => 0]));
        ...
        while ($userdata = $gui->next_user()) {
            ...
            echo $OUTPUT->heading($renderer->user_report_heading($report->user, $courseid, false));

            if ($report->fill_table()) {
                $PAGE->requires->js_call_amd('gradereport_user/gradecategorytoggle', 'init', ["user-report-{$user->id}"]);
                echo \html_writer::div($report->print_table(true), 'user-report-container', ['id' => "user-report-{$user->id}"]);
            }
        }
        $gui->close();
    } else {
        ...
        print_grade_page_head($courseid, 'report', 'user',
            $renderer->user_report_heading($report->user, $courseid),
        ...
        if ($currentgroup && !groups_is_member($currentgroup, $userid)) {
            ...
            if ($report->fill_table()) {
                $PAGE->requires->js_call_amd('gradereport_user/gradecategorytoggle', 'init', ["user-report-{$userid}"]);
                echo \html_writer::div($report->print_table(true), 'user-report-container', ['id' => "user-report-{$userid}"]);
            }
            // Add previous/next user navigation.
            echo $renderer->user_navigation($gui, $userid, $courseid);
        }
    }
} else {
    // Students will see just their own report.
    // Create a report instance.
    $report = new gradereport_user\report\user($courseid, $gpr, $context, $USER->id);
