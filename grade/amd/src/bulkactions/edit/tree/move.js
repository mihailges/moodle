...
/**
 * Class that enables bulk moving of items in the gradebook setup page.
 *
...
import GradebookBulkAction from 'core_grades/bulkactions/bulk_action';
...
const Selectors = {
    ...
    gradeTree: '#destination-selector [role="tree"]',
    gradeTreeSection: '[data-for="sectionnode"][role="treeitem"]',
    gradeTreeItem: '[role="treeitem"]',
    gradeTreeSectionVisibilityToggle: '.collapse-list-link',
    gradeTreeSectionContent: '.collapse-list-item-content',
    bulkMoveInput: 'input[name="bulkmove"]',
    bulkMoveAfterInput: 'input[name="moveafter"]'
};

export default class GradebookEditTreeBulkMove extends GradebookBulkAction {
    ...
    async triggerBulkAction() {
        ...
        this.registerCustomListenerEvents();
    }
    ...
    async renderBulkActionTrigger() {
        const title = await getString('move');
        const icon = await Templates.renderPix('i/dragdrop');
        const templateData = {
            'action': 'move',
            'title': title,
            'icon': icon
        };
        return Templates.render('core_grades/bulkactions/edit/tree/bulk_move_trigger', templateData);
    }
     ...
     */
    async showModal() {
        const courseGradeTree = await this.fetchGradeTree(this.courseID);

        return ModalFactory.create({
            title: await getString('movesitems', 'grades'),
            body: await Templates.render('core_grades/bulkactions/edit/tree/bulk_move_grade_tree', JSON.parse(courseGradeTree)),
            buttons: {
    ...
    moveToNextGradeTreeItem(gradeTreeItem) {
        ...
        const nextGradeTreeItem = this.visibleGradeTreeItems[currentGradeTreeItemIndex + 1];
        if (nextGradeTreeItem) {
            nextGradeTreeItem.focus();
        }
    }
    ...
    moveToPreviousGradeTreeItem(gradeTreeItem) {
        const currentGradeTreeItemIndex = this.visibleGradeTreeItems.findIndex(item => item === gradeTreeItem);
        const previousGradeTreeItem = this.visibleGradeTreeItems[currentGradeTreeItemIndex - 1];
        if (previousGradeTreeItem) {
            previousGradeTreeItem.focus();
        }
    }
    ...
    moveToFirstGradeTreeItem() {
        const firstGradeTreeItem = this.visibleGradeTreeItems[0];
        if (firstGradeTreeItem) {
            firstGradeTreeItem.focus();
        }
    }
    ...
    moveToLastGradeTreeItem() {
        const lastGradeTreeItem = this.visibleGradeTreeItems[this.visibleGradeTreeItems.length - 1];
        if (lastGradeTreeItem) {
            lastGradeTreeItem.focus();
        }
    }
    ...
    setVisibleGradeTreeItems() {
        const gradeTree = this.modal.getBody()[0].querySelector(Selectors.gradeTree);
        const allGradeTreeItems = Array.from(gradeTree.querySelectorAll(Selectors.gradeTreeItem));
