...
const registerEventListeners = () => {
    ...
    $(selectors.userMenu).on('shown.bs.dropdown', function() {
        ...
        userMenu.querySelectorAll(selectors.userMenuCarouselItem).forEach(element => {
            // Resize all non-active carousel items to match the height of the current active (main)
            // carousel item to avoid sizing inconsistencies. This has to be done once the dropdown menu is fully
            // displayed ('shown.bs.dropdown') as the offsetHeight cannot be obtained when the element is hidden.
