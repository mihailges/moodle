...
M.form_filemanager.selectedfiles = [];

M.form_filemanager.lastselected = null;
...
M.form_filemanager.init = function(Y, options) {
    ...
    Y.extend(FileManagerHelper, Y.Base, {
        ...
        setup_buttons: function() {
            ...
            var button_bulkdelete  = this.filemanager.one('.fp-btn-bulkdelete');
            ...
            // setup 'download this folder' button
            button_bulkdelete.on('click', function(e) {
                e.preventDefault();
                var dialog_options = {};
                var params = {};
                params.files = M.form_filemanager.selectedfiles;
                ...
                dialog_options.callback = function(params) {
                    ...
                    this.request({
                        ...
                        callback: function(id, obj, args) {
                            ...
                            args.scope.filecount =  args.scope.filecount - obj.length;
                            args.scope.refresh(obj[0].filepath);
                            if (typeof M.core_formchangechecker != 'undefined') {
                                M.core_formchangechecker.set_form_changed();
                            }
                        }
            ...
            var options = {
                ...
                callback : function(e, node) {
                    ...
                    if (node.type == 'folder') {
                        ...
                        if (e.target.hasClass('filename-checkbox')) {
                            var index = node.filepath + node.filename;
                            if (e.target.hasAttribute('checked')) {
                                e.target.removeAttribute('checked');
                                var index1 = M.form_filemanager.selectedfiles.indexOf(index);
                                M.form_filemanager.selectedfiles.splice(index1, 1);
                                //delete this.selectedfiles[];
                            } else {
                                if (e.shiftKey) {
                                    var multiselected = [];
                                    if (M.form_filemanager.lastselected) {
                                        var files = [];
                                        list.forEach(function(element){
                                            files.push(element.filepath + element.filename);
                                        });

                                        var lastselectedindex = files.indexOf(M.form_filemanager.lastselected);
                                        var presentselectedindex = files.indexOf(index);
                                        var start;
                                        var end;
                                        if (lastselectedindex < presentselectedindex) {
                                            start = lastselectedindex;
                                            end = presentselectedindex;
                                        } else {
                                            start = presentselectedindex;
                                            end = lastselectedindex;
                                        }

                                        multiselected = files.slice(start, end);

                                        this.filemanager.all('.filename-checkbox').each(function (node) {
                                            if (multiselected.indexOf(node.get('value')) !== -1) {
                                                node.setAttribute('checked', 'checked');
                                                if (M.form_filemanager.selectedfiles.indexOf(node.get('value')) == -1) {
                                                    M.form_filemanager.selectedfiles.push(node.get('value'));
                                                    M.form_filemanager.lastselected = node.get('value');
                                                }
                                            }
                                        });
                                    }
                                    ...
                                    //this.selectedfiles[index] = {"filepath": node.filepath, "filename" : node.filename};
                                    M.form_filemanager.selectedfiles.push(index);
                                    M.form_filemanager.lastselected = index;
                                }
                                M.form_filemanager.lastselected = index;

                            }
                            console.log(M.form_filemanager.selectedfiles);
                        } else {
        ...
        multi_select_files: function() {
            var files = this.filemanager.all('.filename-checkbox');
        },
        render: function() {
            this.print_path();
            this.view_files();
