...
class grade_letters_action_bar extends action_bar {
    ...
    public function export_for_template(\renderer_base $output): array {
        ...
        $generalnavselector = new general_action_bar($this->courseid,
            new moodle_url('/grade/edit/letter/index.php', ['id' => $context->id]), 'letter', 'view');
        $data = $generalnavselector->export_for_template($output);

        // If the user has capabilities to manage grade letters, add a button to the action bar with a link to the
        // 'edit grade letters' page.
        if (has_capability('moodle/grade:manageletters', $context)) {
            $editbuttonlink = new moodle_url('/grade/edit/letter/index.php',
                ['id' => $context->id, 'edit' => 1]);
