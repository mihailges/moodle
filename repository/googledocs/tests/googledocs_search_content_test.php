...
/**
 * Repository content search unit tests.
 *
...
/**
 * Tests for the search content class.
 *
 * @package    repository_googledocs
 * @copyright  2021 Mihail Geshoski <mihail@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class repository_googledocs_search_testcase extends googledocs_content_testcase {
    ...
    public function test_get_content_nodes(string $query, bool $sortcontent, array $filterextensions,
        ...
        // Assert that the call() method is being called with the given arguments (first instance) to fetch the
        // existing shared drives. Define the returned data object by this call.
        ...
        // Assert that the call() method is being called with the given arguments (second instance) to fetch the
        // relevant contents that match the search criteria. Define the returned data object by this call.
        $servicemock->expects($this->at(1))
            ->method('call')
            ->with('list', $searchparams)
            ->willReturn((object)[
                'files' => $searccontents
            ]);
    ...
    public function test_get_navigation(string $nodepath, array $expected) {
        ...
        $searchcontentobj = new googledocs_content_search($servicemock, $nodepath);
        $navigation = $searchcontentobj->get_navigation();
            ...
            'Return navigation nodes array from path where all nodes have a name.' =>
                [
                    ...
                    [
                        ...
                        [
                            'name' => "{$searchforstring} 'test'",
                            'path' => "{$rootid}|Google+Drive/{$searchnodeid}|" .
                                urlencode("{$searchforstring} 'test'")
                        ]
                    ]
                ],
            'Return navigation nodes array from path where some nodes do not have a name.' =>
                [
                    "{$rootid}/{$searchnodeid}|" . urlencode("{$searchforstring} 'test'"),
                    [
                        [
                            'name' => $rootid,
                            'path' => "{$rootid}|{$rootid}"
                        ...
                        [
                            'name' => "{$searchforstring} 'test'",
                            'path' => "{$rootid}|{$rootid}/{$searchnodeid}|" .
                                urlencode("{$searchforstring} 'test'")
                        ]
                    ],
                ],
