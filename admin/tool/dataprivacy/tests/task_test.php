...
class tool_dataprivacy_task_testcase extends data_privacy_testcase {
    ...
    /**
     * Ensure that a delete data request for pre-existing deleted users
     * is created when there are existing finished delete data requests
     * for that particular user.
     * @group qtesttt
     */
    public function test_delete_existing_deleted_users_task_existing_finished_delete_data_requests() {
        ...
        $this->execute_task('tool_dataprivacy\task\delete_existing_deleted_users');
        // After running the scheduled task, the user should have two delete data requests.
        $this->assertCount(2, api::get_data_requests($user->id, [],
                [api::DATAREQUEST_TYPE_DELETE]));
        // The user should have a newly created delete data request.
        $this->assertCount(1, api::get_data_requests($user->id,
                [api::DATAREQUEST_STATUS_PENDING], [api::DATAREQUEST_TYPE_DELETE]));
    }
    ...
    /**
     * Ensure that a delete data request for pre-existing deleted users
     * is not created when there already are ongoing and finished delete
     * data requests for that particular user.
     * @group qtesttt
     */
    public function test_delete_existing_deleted_users_task_existing_finished_and_ongoing_delete_data_requests() {
        $this->resetAfterTest();
        $this->setAdminUser();
        // Create a user.
        $user = $this->getDataGenerator()->create_user();
        $this->setUser($user);
        // Create delete data request for the user.
        $datarequest1 = api::create_data_request($user->id, api::DATAREQUEST_TYPE_DELETE);
        $requestid1 = $datarequest1->get('id');
        // Update the status of the delete data request to 'Cancelled'.
        api::update_request_status($requestid1, api::DATAREQUEST_STATUS_CANCELLED);
        // Create delete data request for the user.
        $datarequest2 = api::create_data_request($user->id, api::DATAREQUEST_TYPE_DELETE);
        $requestid2 = $datarequest2->get('id');
        // Update the status of the delete data request to 'Awaiting for approval'.
        api::update_request_status($requestid2, api::DATAREQUEST_STATUS_AWAITING_APPROVAL);

        // The user should have two delete data requests.
        $this->assertCount(2, api::get_data_requests($user->id, [],
                [api::DATAREQUEST_TYPE_DELETE]));
        // The user should have an ongoing data request.
        $this->assertTrue(api::has_ongoing_request($user->id, api::DATAREQUEST_TYPE_DELETE));

        $this->setAdminUser();
        // Delete the user.
        delete_user($user);
        // The user should still have two pre-existing delete data request.
        $this->assertCount(2, api::get_data_requests($user->id, [],
                [api::DATAREQUEST_TYPE_DELETE]));
