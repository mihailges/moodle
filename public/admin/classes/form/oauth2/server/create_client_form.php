...
class create_client_form extends base_client_form {
    ...
    public function definition(): void {
        ...
        foreach ($typeoptions as $val => $data) {
            $label = $this->create_label($data['name'], $data['desc'], $data['icon']);
     ...
     * @param string|null $iconclass Optional icon class.
     * @return string Generated HTML.
     */
    private function create_label(string $name, string $description, ?string $iconclass = null): string {
        $icondiv = '';

        if ($iconclass) {
            $icon = \html_writer::tag('i', '', ['class' => "{$iconclass} me-1 text-secondary", 'aria-hidden' => 'true']);
            $icondiv = \html_writer::div($icon, 'me-1 text-secondary');
        }

        $namediv = \html_writer::div($name, 'fw-semibold');
        $descriptiondiv = \html_writer::div($description, 'text-muted small');
        $contentdiv = \html_writer::div($namediv . $descriptiondiv);

        return \html_writer::div($icondiv . $contentdiv, 'd-inline-flex align-items-start ms-1');
    }
}
