...
class provider implements
     ...
     * @param string $itemtype the type of the favorited items.
     */
    ...
    public static function get_favourites_info_for_user(int $userid, \context $context,
        ...
        if (!$favorited = $DB->get_record('favourite', $params)) {
            return;
        }

        return [
            'starred' => transform::yesno(true),
            'ordering' => $favorited->ordering,
            'timecreated' => transform::datetime($favorited->timecreated),
            'timemodified' => transform::datetime($favorited->timemodified)
        ];
    }
