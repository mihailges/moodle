...
//?????????
        ...
        // $DB->delete('h5p_libraries_languages', array('libraryid' => $library->id));
...
//        global $DB;
//
//        return $DB->count_records('h5p', [$DB->sql_compare_text('filtered') => '']);
    }
...
//
//        $count = array();
//        $DB->get_records_select('h5p_counters', array('library_name'));
//
//        $sql = "SELECT id, libraryname AS name, libraryversion AS version, num
//                  FROM {h5p_counters}
//                 WHERE type = ?";
//        // Get the counts for the given type of event.
//        $records = $DB->get_records_sql($sql, array($type));
//
...
//        $DB->delete_records('h5p_libraries_hub_cache');
//        foreach ($contenttypecache->contentTypes as $ct) {
//            $DB->insert_record('h5p_libraries_hub_cache', (object) array(
//                'machinename'      => $ct->id,
//                'majorversion'     => $ct->version->major,
//                'minorversion'     => $ct->version->minor,
//                'patchversion'     => $ct->version->patch,
//                'h5pmajorversion' => $ct->coreApiVersionNeeded->major,
//                'h5pminorversion' => $ct->coreApiVersionNeeded->minor,
//                'title'             => $ct->title,
//                'summary'           => $ct->summary,
//                'description'       => $ct->description,
//                'icon'              => $ct->icon,
//                'createdat'        => (new \DateTime($ct->createdAt))->getTimestamp(),
//                'updatedat'        => (new \DateTime($ct->updatedAt))->getTimestamp(),
//                'isrecommended'    => $ct->isRecommended === true ? 1 : 0,
...
//        $sql = "SELECT l1.id AS library_id,
//                       l1.machinename,
//                       l1.majorversion,
//                       l1.minorversion,
//                       l1.patchversion,
