...
class gradereport_user41 extends grade_report {
     ...
     */
    public function __construct($courseid, $gpr, $context, $userid, $viewasuser = null, $itemtype = null, $itemid = null) {
...
//
//        if ($element['type'] == 'category' && $element['prev'] != 0) {
//            $count += 1;
//        }
//
...
//        }
        ...
        if ($type == 'item' or $type == 'categoryitem' or $type == 'courseitem') {
            ...
            if (!$hide) {
                ...
                if ($this->switch) { // Alter style based on whether aggregation is first or last.
                    $class .= ($type == 'categoryitem' or $type == 'courseitem') ? " d$depth baggt b2b" : " item b1b";

                } else {
                    $class .= ($type == 'categoryitem' or $type == 'courseitem') ? " d$depth baggb " : " item b1b";
                }
                if ($type == 'categoryitem' or $type == 'courseitem') {
                    $header_cat = "cat_{$grade_object->iteminstance}_{$this->user->id}";
                    $issingleitem = $element['prev'] === 0;
                    $content = $fullname;
                } else {
                if ($grade_object->itemtype === 'mod') {
                    $itemtype = get_string('modulename', $grade_object->itemmodule);
                } else {
                    $itemtype = get_string('manualitem', 'grades');
                }
                $type = html_writer::div($itemtype, 'text-uppercase small dimmed_text');
                $name = html_writer::div($fullname);
                $content = html_writer::div($type . $name, '');
            }
            ...
            $parentcategoryids = array_filter(explode('/', $grade_object->parent_category->path));
        ...
        if ($type == 'category') {
            ...
            $collapse = html_writer::span($OUTPUT->pix_icon('t/expandedchevron', get_string('delete')), 'collapsed text-nowrap', ['title' => get_string('collapse')]);
            $expand = html_writer::span($OUTPUT->pix_icon('t/collapsedchevron', get_string('delete')), 'expanded text-nowrap', ['title' => get_string('expand')]);

            $anchor = html_writer::link('#', $collapse . $expand, ['aria-expanded' => 'true', 'role' => 'button', 'data-target' => ".cat_{$grade_object->id}", 'class' => 'btn btn-icon toggle-category']);
            $data['itemname']['content'] = $anchor . $fullname;
            $data['itemname']['celltype'] = 'th';
            $data['itemname']['id'] = "cat_{$grade_object->id}_{$this->user->id}";

            $parentcategoryids = array_diff(array_filter(explode('/', $grade_object->path)), [$grade_object->id]);
        }
        ...
        $data['parentcategories'] = $parentcategoryids;
        $data['issingleitem'] = $issingleitem;

        $this->tabledata[] = $data;

        if ($element['type'] == 'category') {
            // if ($depth !== 1) {
            $emptyrow['leader']['class'] = $class . " d$depth b1t b2b b1l";
            $emptyrow['leader']['rowspan'] = $element['rowspan'];
            // }

            if ($this->switch) { // alter style based on whether aggregation is first or last
                $emptyrow['itemname']['class'] = $class . " d$depth b1b b1t column-0 empty'";
            } else {
                $emptyrow['itemname']['class'] = $class . " d$depth b2t";
            }
            $emptyrow['itemname']['colspan'] = ($this->maxdepth - $depth + count($this->tablecolumns) - 1);
            $emptyrow['itemname']['content'] = '';
            $emptyrow['itemname']['celltype'] = 'th';
            $emptyrow['itemname']['id'] = "cat_{$grade_object->id}_{$this->user->id}";
            $emptyrow['parentcategories'] = array_filter(explode('/', $grade_object->path));
    ...
    public function print_table($return=false) {
        ...
        for ($i = 0; $i < count($this->tabledata); $i++) {
            $parentcaregories = implode(" ", array_map(function($parentcategoryid) {
                return "cat_{$parentcategoryid}";
            }, $this->tabledata[$i]['parentcategories']));

            $singleitem = $this->tabledata[$i]['issingleitem'] ? 'singleitem' : '';

            $html .= "<tr class='{$parentcaregories} {$singleitem}'>\n";
...
function gradereport_user41_settings_definition(&$mform) {
    global $CFG;
 ...
 */
function gradereport_user41_profilereport($course, $user, $viewasuser = false) {
    global $OUTPUT;
 ...
 */
function gradereport_user41_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    global $CFG, $USER;
    if (empty($course)) {
