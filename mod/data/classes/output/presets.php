...
class presets implements templatable, renderable {
    ...
    private function get_presets_table(): string {
        ...
        $presetstable->align = ['center', 'left', 'left', 'left'];
        $presetstable->size = ['1%', '90%', '1%', '1%'];
        ...
        foreach ($this->presets as $preset) {
            ...
            $deleteaction = '';
            if ($this->manage) {
                $downloadaction = $OUTPUT->action_icon(new moodle_url(''),
                    new \pix_icon('t/download', get_string('download')));

                if (data_user_can_delete_preset($PAGE->context, $preset) && $preset->name != 'Image gallery') {
                    $deleteaction = $OUTPUT->action_icon(new moodle_url('/mod/data/presets.php',
                        ['d' => $this->id, 'fullname' => "{$preset->userid}/{$preset->shortname}", 'action' => 'confirmdelete',
                            'sesskey' => sesskey()]), new \pix_icon('t/delete', get_string('delete')));
                }
            }
