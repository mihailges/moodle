...
class core_course_privacy_testcase extends \core_privacy\tests\provider_testcase {
    ...
    public function test_get_contexts_for_userid() {
        ...
        // Make sure the course context is being returned.
        $contextlist = \core_course\privacy\provider::get_contexts_for_userid($user->id);
    ...
    public function test_delete_data_for_user() {
        ...
        // Ensure user1 and user2 are returned in the course context.
        $userlist = new \core_privacy\local\request\userlist($this->coursecontext, 'core_course');
        \core_course\privacy\provider::get_users_in_context($userlist);
        $actual = $userlist->get_userids();
        ...
        // Ensure user1's data is deleted and only user2 is returned in the course context.
        $userlist = new \core_privacy\local\request\userlist($this->coursecontext, 'core_course');
        \core_course\privacy\provider::get_users_in_context($userlist);
        $actual = $userlist->get_userids();
        $expected = [$user2->id];
