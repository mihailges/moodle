...
namespace gradereport_singleneo\external;
...
/**
 * External grade report singleneo API
 *
...
class gradeitems extends core_course_external {
    ...
    protected static function get_grade_items_for_search_widget(int $courseid): array {
        ...
        // We do not want to should the course as a whole.
        $gradeableitems = array_filter($gradeableitems, function($gradeitem){
            return $gradeitem->itemtype !== 'course';
        });
        $gradeitems = array_map(function ($gradeitem) use ($PAGE, $USER) {
            $item = new \stdClass();
