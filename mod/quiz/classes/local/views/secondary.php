...
/**
 * Custom secondary navigation for the quiz activity.
 *
 * This would help the activity tab to be activated when
 * edit or preview pages are navigated to by the user.
 *
 * @package   mod_quiz
...
class secondary extends core_view_secondary {
    ...
    protected function load_module_navigation(): void {
        ...
        if ($mainnode) {
            ...
            if (!$setactive) {
                // An additional check to see if the url is preview or edit.
                // If $PAGE->url is edit or preview, then activity tab needs to be active.
                $urls = [
                        new \moodle_url('/mod/' . $this->page->activityname . '/edit.php'),
                        new \moodle_url('/mod/' . $this->page->activityname . '/attempt.php'),
                ];
                foreach ($urls as $newurl) {
                    $setactive = $newurl->compare($this->page->url, URL_MATCH_BASE);
                    if ($setactive) {
                        break;
                    }
                }
