...
class helper {
    ...
    /**
     * Returns the relevant googledocs content node class based on the file's mimetype.
     *
     * @param \stdClass $content The google drive content object
     * @param string $path The current path
     * @return node The content node object
     */
    public static function get_content_node(\stdClass $content, string $path): node {
        // Return the relevant content browser class based on the ID of the current path node.
        switch ($content->mimeType) {
            case 'application/vnd.google-apps.folder':
                return new folder_node($content, $path);
            default:
                return new file_node($content);
        }
    }
