...
class create_template_form_test extends \advanced_testcase {
    /** @var \stdClass The course we have created */
    private $course;
    /** @var array The feedback id along with the templates generated from it.*/
    private $feedback = [];
    /** @var \stdClass The student enrolled in the course.*/
    private $user;
    /** @var \stdClass The teacher enrolled in the course.*/
    private $teacher;
    /** @var \stdClass The manager at a site level.*/
    private $manager;
    /** @var \stdClass The manager role at a site level.*/
    private $managerrole;
    ...
    public function setUp(): void {
        ...
        $feedback = $this->getDataGenerator()->create_module('feedback', ['course' => $this->course->id]);
        $cm = get_coursemodule_from_instance('feedback', $feedback->id, $this->course->id);
    ...
    public function test_createtemplate_form_with_modified_capabilities(array $unassignedroles, bool $accessallowed,
        ...
        foreach ($unassignedroles as $role) {
            unassign_capability($role, $this->managerrole->id);
        }
        $data = [
            'id' => $this->feedback['id'],
