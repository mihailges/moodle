...
function lti_add_instance($lti, $mform) {
    ...
    $resourcelinkmanager->create_resource_link($lti->typeid, $lti->coursemodule, $lti->toolurl, $lti->name, $lti->intro,
        $lti->introformat, $lti->instructorchoiceacceptgrades, $lti->servicesalt, $launchcontainer, $icon, $customparams);
...
function lti_update_instance($lti, $mform) {
    ...
    return $resourcelinkmanager->update_resource_link($lti->coursemodule, $ltiresourcelinkformvalues);
}
