...
abstract class contentbank_browser {
    ...
    protected function get_child_contexts(): array {
        ...
        if (empty($allowedcontextlevels = $this->allowed_child_context_levels())) {
            ...
            return $allowedcontextlevels;
        }

        $select = 'depth = :depth';
        $params['depth'] = $this->context->depth + 1;

        for ($i = 0; $i <= $last = count($allowedcontextlevels) - 1; $i++) {
            $select .= ($i === 0) ? ' AND (' : ' OR ';
            $select .= "contextlevel = :contextlevel{$i}";
            if ($i == $last) {
                $select .= ')';
            }
            $params["contextlevel{$i}"] = $allowedcontextlevels[$i];
        }
    ...
    /**
     * Generate folder nodes for the relevant child contexts which can be accessed/viewed by the used.
     *
    ...
    private function get_contentbank_files(): array {
        ...
        // Return all content bank file nodes from the current context which can be accessed by the user following the
