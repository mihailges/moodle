...
if ($mode != 'csstemplate' and $mode != 'jstemplate') {
    if ($usehtmleditor) {
        ...
        $toggletemplateeditor = html_writer::checkbox('useeditor', 1, true,
            get_string('editorenable', 'data'),
            ['data-action' => $url->out(false)], ['class' => 'pl-2']);
    } else {
        $url = new moodle_url($PAGE->url, ['useeditor' => true]);
        $toggletemplateeditor = html_writer::checkbox('useeditor', 1, false,
            get_string('editorenable', 'data'),
            ['id' => 'enabletemplateeditor', 'data-action' => $url->out(false)], ['class' => 'pl-2']);
    }
}
...
$PAGE->requires->js_call_amd('mod_data/templateseditor', 'init');
