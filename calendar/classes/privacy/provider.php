        ...
        \core_privacy\local\request\user_preference_provider
{
    ...
    public static function get_users_in_context(userlist $userlist) {
        ...
        $sql = "SELECT e.userid
                  ...
                  JOIN {modules} m
                       ON m.id = cm.module
                  JOIN {event} e
                       ON e.modulename = m.name AND e.courseid = cm.course AND e.instance = cm.instance
                  JOIN {context} ctx
                       ON ctx.instanceid = cm.id AND ctx.contextlevel = :modulecontext
        ...
        $sql = "SELECT e.userid
                  FROM {event} e
                  JOIN {context} ctx ON
                       (ctx.contextlevel = :sitecontext AND e.eventtype = 'site') OR
                       (ctx.contextlevel = :categorycontext AND ctx.instanceid = e.categoryid AND e.eventtype = 'category') OR
                       (ctx.contextlevel = :coursecontext AND ctx.instanceid = e.courseid AND e.eventtype = 'course') OR
                       (ctx.contextlevel = :groupcontext AND ctx.instanceid = e.courseid AND e.eventtype = 'group') OR
                       (ctx.contextlevel = :usercontext AND ctx.instanceid = e.userid AND e.eventtype = 'user')
                 WHERE ctx.id = :contextid";
        ...
        $sql = "SELECT s.userid
                  ...
                  JOIN {context} ctx ON
                       (ctx.contextlevel = :sitecontext AND s.eventtype = 'site') OR
                       (ctx.instanceid = s.categoryid AND ctx.contextlevel = :categorycontext AND s.eventtype = 'category') OR
                       (ctx.instanceid = s.courseid AND ctx.contextlevel = :coursecontext AND s.eventtype = 'course') OR
                       (ctx.instanceid = s.courseid AND ctx.contextlevel = :groupcontext AND s.eventtype = 'group') OR
                       (ctx.instanceid = s.userid AND ctx.contextlevel = :usercontext AND s.eventtype = 'user')
                 WHERE ctx.id = :contextid";
    ...
    public static function delete_data_for_users(approved_userlist $userlist) {
        ...
        // Delete all Calendar Events in the specified context in batches.
        if ($eventids = array_keys(self::get_calendar_event_ids_by_userlist($userlist))) {
            self::delete_batch_records('event', 'id', $eventids);
        }

        // Delete all Calendar Subscriptions in the specified context in batches.
        if ($subscriptionids = array_keys(self::get_calendar_subscription_ids_by_userlist($userlist))) {
            self::delete_batch_records('event_subscriptions', 'id', $subscriptionids);
        }
    }
    ...
    /**
     * Helper function to return all Calendar Event id results for a specified approved user list.
     *
     * @param approved_userlist $userlist
     * @return array
     * @throws \dml_exception
     */
    protected static function get_calendar_event_ids_by_userlist(approved_userlist $userlist) {
        global $DB;
    ...
    /**
     * Helper function to return all Calendar Subscription id results for a specified approved user list.
     *
     * @param approved_userlist $userlist
     * @return array
     * @throws \dml_exception
     */
    protected static function get_calendar_subscription_ids_by_userlist(approved_userlist $userlist) {
        global $DB;
