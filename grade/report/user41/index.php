...
$PAGE->set_url(new moodle_url('/grade/report/user41/index.php', ['id' => $courseid]));

if ($userview == 0) {
    $userview = get_user_preferences('gradereport_user41_view_user', GRADE_REPORT_USER41_VIEW_USER);
} else {
    set_user_preference('gradereport_user41_view_user', $userview);
}
...
if (has_capability('moodle/grade:viewall', $context)) { // Teachers will see all student reports.
    ...
    $renderer = $PAGE->get_renderer('gradereport_user41');
    ...
    if (is_null($userid)) {
        print_grade_page_head($courseid, 'report', 'user', ' ',
            ...
            false);
        echo $renderer->render_zero_state();
    } else if ($userid == 0) {
        ...
        // Print header.
        $actionbar = new \gradereport_user41\output\user_report_action_bar($context,
            new moodle_url('/grade/report/user41/index.php', ['id' => $courseid]), 'report', 'user41');

        print_grade_page_head($courseid, 'report', 'user', null, null, null, null,
        ...
        while ($userdata = $gui->next_user()) {
            $user = $userdata->user;
            $report = new gradereport_user41($courseid, $gpr, $context, $user->id, $viewasuser);
            ...
            if ($report->fill_table()) {
                $PAGE->requires->js_call_amd('gradereport_user/gradecategorytoggle', 'init');
                echo html_writer::div($report->print_table(true), 'user-grade-container');
            }
            echo "<p style = 'page-break-after: always;'></p>";
        }
        $gui->close();
    } else { // Only show one user's report
        $report = new gradereport_user41($courseid, $gpr, $context, $userid, $viewasuser);

        $PAGE->requires->js_call_amd('gradereport_user41/user', 'init');

        $actionbar = new \gradereport_user41\output\action_bar($context, $report->user);

        $studentnamelink = html_writer::link(new moodle_url('/user/view.php', array('id' => $report->user->id, 'course' => $courseid)), fullname($report->user));
        print_grade_page_head($courseid, 'report', 'user', get_string('pluginname', 'gradereport_user') . ' - ' . $studentnamelink,
            false, false, true, null, null, $report->user, $actionbar, false);
        ...
        if ($currentgroup and !groups_is_member($currentgroup, $userid)) {
            ...
            if ($report->fill_table()) {
                $PAGE->requires->js_call_amd('gradereport_user41/gradecategorytoggle', 'init');
                echo html_writer::div($report->print_table(true), 'user-grade-container');
            }
        }
    }
