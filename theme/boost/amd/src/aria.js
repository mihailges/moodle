...
const comboboxFix = () => {
    ...
    document.addEventListener('keydown', e => {
        if (e.target.matches('.dropdown [role="combobox"]')) {
            ...
            // Under the special case that the dropdown menu is being shown as teh result of they key press (like when the user
    ...
    document.addEventListener('click', e => {
        if (e.target.matches('.dropdown [role="listbox"] [role="option"]')) {
            const option = e.target;
            const combobox = option.closest('.dropdown').querySelector('[role="combobox"]');
            combobox.focus();
