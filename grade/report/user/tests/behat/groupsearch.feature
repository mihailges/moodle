...
Feature: Group searching functionality within the user report.
  ...
  Scenario: A teacher can search for and find a group to find a user in
    ...
    And I confirm "Tutor group" in "group" search widget exists
    And I confirm "Marker group" in "group" search widget exists
    When I set the field "searchinput" to "tutor"
    And I wait "1" seconds
    Then I confirm "Tutor group" in "group" search widget exists
    And I confirm "Marker group" in "group" search widget does not exist
  ...
  Scenario: A teacher can only see the group members in the 'user' search widget after selecting a group option
    ...
    And I confirm "Student 1" in "user" search widget exists
    And I confirm "Student 2" in "user" search widget exists
    ...
    And I click on ".search-widget[data-searchtype='user']" "css_element"
    Then I confirm "Student 1" in "user" search widget exists
    And I confirm "Student 2" in "user" search widget does not exist
    And I click on "Tutor group" in the "group" search widget
    And I click on ".search-widget[data-searchtype='user']" "css_element"
    And I confirm "Student 1" in "user" search widget does not exist
    And I confirm "Student 2" in "user" search widget does not exist
