...
Feature: Bulk remove submissions
  In order to restart an assignment for multiple students
  As a teacher
  I need to bulk remove student submissions at any time
  ...
  @javascript
  Scenario: Notification should be displayed when users without a submission are selected for submission bulk removal
    Given I log in as "admin"
    And I set the following system permissions of "Teacher" role:
      | capability                     | permission |
      | mod/assign:editothersubmission | Allow      |
    ...
    And I click on "Go" "button" confirming the dialogue
    Then I should not see "I'm the student1 submission"
    And I should see "A submission submitted by Student 2 could not be found"
