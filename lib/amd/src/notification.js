...
function(Y, $, log) {
    var notificationModule = {
        ...
        addNotification: function(notification) {
            ...
            // The 'flash' property defines whether the notification can be automatically removed.
            // Meaning, 'flash' notifications are defined as notifications that simply describe the status of a user action
            // without providing additional, more meaningful context to the user that
            notification = $.extend({
                    closebutton:    true,
                    announce:       true,
                    type:           'error',
            ...
            var flashNotification = null;

            // When adding a flash notification.
            if (notification.flash) {
                // Add a new or update an existing "info" flash notification message.


                // If the flash notification of the desired type is already present in the notification area we should
                // simply update it in order to improve the transition.
                // We should also remove the other flash notifications.
                $.each(allFlashNotifications, function(index, flashNotificationElement) {
                    //
                    if ($(flashNotificationElement).data("type") == notification.type) {
                        flashNotification = flashNotificationElement;
        ...
        clearNotificationElements: function(notificationElements) {
            ...
            var targetRegion = $('#' + notificationModule.fieldName);
            $.each(notificationElements, function(index, notificationElement) {
                // Improve the transition when clearing out the notification element.
                targetRegion.find(notificationElement).fadeOut(500, function() {
                    $(this).remove();
                });
            });
        ...
        updateNotification: function(notificationElement, content) {
            ...
            notification.fadeOut(500, function() {
                ...
                // Move (prepend) the updated notification at the top of the notification list if it is not already.
    ...
    return /** @alias module:core/notification */{
         ...
         *
         * @method clearNotificationElements
         * @param {Object} notificationElements    The jquery objects that represent the notification elements.
         ...
         * @param {Object} notificationAttr           The notification attributes.
         * @param {string} notificationAttr.type      The type of the notification.
         * @param {Boolean} notificationAttr.flash    Whether the notification is a flash notification.
         */
