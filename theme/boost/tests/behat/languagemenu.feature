...
Feature: Language selector menu
  ...
  Scenario: Logged user is presented with a language selector which is placed within the user menu
    ...
    And ".langmenu [role='menu']" "css_element" should not exist in the ".navbar" "css_element"
    # The language selector is present within the user menu.
    When I follow "Language" in the user menu
    Then ".usermenu .dropdown-menu .submenu" "css_element" should be visible
    And I should see "Language selector" in the ".usermenu .dropdown-menu .submenu" "css_element"
    And "English ‎(en)‎" "link" should exist in the ".usermenu .dropdown-menu .submenu" "css_element"
    And "English (pirate) ‎(en_ar)‎" "link" should exist in the ".usermenu .dropdown-menu .submenu" "css_element"
  ...
  Scenario: Non-logged user is presented with a language selector which is placed within the navbar
    ...
    And ".langmenu [role='menu']" "css_element" should exist in the ".navbar" "css_element"
    When I click on ".langmenu a#lang-menu-toggle" "css_element" in the ".langmenu" "css_element"
    Then ".langmenu [role='menu']" "css_element" should be visible
    And "English ‎(en)‎" "link" should exist in the ".langmenu [role='menu']" "css_element"
    And "English (pirate) ‎(en_ar)‎" "link" should exist in the ".langmenu [role='menu']" "css_element"
  ...
  Scenario: Logged user is not presented with a language selector in a course if a language is forced in that context
    ...
    Then "Language" "link" should not exist in the ".usermenu [role='menu']" "css_element"
    # The language selector is present within the user menu in other contexts.
    And I am on site homepage
    And "Language" "link" should exist in the ".usermenu [role='menu']" "css_element"
  ...
  Scenario: Non-logged user is not presented with a language selector if there is less than two installed languages
    ...
    Then ".langmenu [role='menu']" "css_element" should not exist in the ".navbar" "css_element"
