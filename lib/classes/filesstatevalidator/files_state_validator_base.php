...
/**
 * Contains an abstract base class definition for files state validator.
 *
 * @package   core
 ...
 * @author    Mihail Geshoski <mihail@moodle.com>
 */
abstract class files_state_validator_base {
     ...
     */
    public function validate_revision($currentrevision, $dbrevision) {
        // Check if the revision number has changed
        if ($currentrevision != $dbrevision) {
            $warnings[] = $this->return_warning_message();

            return $this->output(true, $warnings);
        }
        return $this->output(false, []);
    }

    private function output($status, $warnings) {
