...
final class client_repository_test extends \advanced_testcase {
     ...
     * @param array $redirecturis The URIs to insert for the client.
    ...
    public function test_get_client_entity(
        ...
        if (!empty($clientdata)) {
            ...
            $clientid = $DB->insert_record('oauth2_server_clients', (object)$clientdata);
        ...
        $clientIdentifier = $clientdata['clientidentifier'] ?? 'non-existent';
        ...
        if ($expectednull) {
            ...
            $this->assertSame($expectedproperties['redirecturi'], $client->getRedirectUri());
        }
    }
    ...
    public static function get_client_entity_provider(): array {
        return [
            ...
            'active, confidential client with single redirect uri' => [
                [
                    'name' => 'Active Client',
                    'clientidentifier' => 'client-active-1',
            ...
            'revoked public client with multiple redirect uris' => [
                [
                    'name' => 'Revoked Client',
                    'clientidentifier' => 'client-revoked-1',
                    'description' => null,
                    'status' => client_entity::STATUS_REVOKED,
                    'isconfidential' => 0,
                    'timecreated' => time(),
                ],
                ['https://example.test/uri1', 'https://example.test/uri2'],
                false,
                [
                    'identifier' => 'client-revoked-1',
                    'name' => 'Revoked Client',
                    'description' => null,
                    'status' => client_entity::STATUS_REVOKED,
    ...
    public static function validate_client_provider(): array {
        ...
        return [
            ...
            'public client matches anything without secret check' => [
                $activepublic, null, 0, 'client-pub', null, 'authorization_code', true
            ],
            'confidential client with correct secret and matching status' => [
                $activeconf, 'supersecret', client_entity::STATUS_ACTIVE, 'client-conf', 'supersecret', 'authorization_code', true
            ],
            'confidential client with wrong secret' => [
                $activeconf, 'supersecret', client_entity::STATUS_ACTIVE, 'client-conf', 'wrongsecret', 'authorization_code', false
            ],
            'confidential client with unmatching revoked status' => [
                $activeconf, 'supersecret', 0, 'client-conf', 'supersecret', 'authorization_code', false
            ],
            'confidential client with empty secret input' => [
                $activeconf, 'supersecret', client_entity::STATUS_ACTIVE, 'client-conf', null, 'authorization_code', false
            ],
        ];
    }
}
