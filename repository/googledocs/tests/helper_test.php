...
/**
 * Google drive repository helper unit tests.
 *
...
/**
 * Tests for the Google Drive repository helper class.
 *
 * @package    repository_googledocs
 * @copyright  2021 Mihail Geshoski <mihail@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class repository_googledocs_helper_testcase extends repository_googledocs_testcase {
    ...
    public function get_browser_provider(): array {
        ...
        return [
            'Repository root level path.' =>
                [
                    "{$rootid}|Google+Drive",
                    'repository_googledocs\browser\googledocs_root_content'
                ],
            'My drive path.' =>
                [
                    "{$rootid}|Google+Drive/{$mydriveid}|My+Drive",
                    'repository_googledocs\browser\googledocs_drive_content'
                ],
            'Shared drives root path.' =>
                [
                    "{$rootid}|Google+Drive/{$shareddrivesid}|Shared+Drives",
                    'repository_googledocs\browser\googledocs_shared_drives_content'
                ],
     ...
     * @param \stdClass $response The response returned by the API call
     */
    public function test_request(string $api, array $params, \stdClass $response) {
        // Mock the service object.
        $servicemock = $this->getMockBuilder('repository_googledocs\rest')->disableOriginalConstructor()
            ->getMock();

        // Assert that the call() method is being called with the given arguments.
        // Define the response returned by this call.
        $servicemock->expects($this->once())
            ->method('call')
            ->with($api, $params)
            ->willReturn($response);

        // Assert that the response returned by the API call is also returned by the request() method.
        $this->assertEquals($response, helper::request($servicemock, $api, $params));
    }

    /**
     * Data provider for test_request().
     *
     * @return array
     */
    public function request_provider(): array {
        ...
        return [
            'The API call (list) response returns contents.' =>
                [
                    'list',
                    [],
                    (object)[
                        'files' => [
                            $this->create_google_drive_folder_object('1c4ad262c65333', 'Folder'),
                            $this->create_google_drive_file_object('d85b21c0f86cb0', 'File.pdf',
                                'application/pdf', 'pdf'),
                        ]
                    ]
                ],
            'The API call (shared_drives_list) response returns contents.' =>
                [
                    'shared_drives_list',
                    [],
                    (object)[
                        ...
                        'drives' => [
                            $this->create_google_drive_shared_drive_object('1c4ad262c65333', 'Shared drive 1'),
                            $this->create_google_drive_shared_drive_object('d85b21c0f86cb0', 'Shared drive 2')
                        ]
                    ]
                ],
