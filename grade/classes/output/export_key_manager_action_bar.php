...
class export_key_manager_action_bar extends action_bar {
    ...
    public function export_for_template(\renderer_base $output): array {
        ...
        $exportnavselectors = new export_action_bar($this->courseid,
            new moodle_url('/grade/export/keymanager.php', ['id' => $this->courseid]), 'keymanager');
        $data = $exportnavselectors->export_for_template($output);

        // Add a button to the action bar with a link to the 'add user key' page.
        $adduserkeylink = new moodle_url('/grade/export/key.php', ['courseid' => $this->courseid]);
