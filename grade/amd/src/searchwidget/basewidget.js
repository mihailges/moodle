 ...
 * @param {Promise|null} footerPromise  The promise from the callee of the contents to place in the modal footer.
 ...
 */
export const init = (bodyPromise, footerPromise, data, searchFunc, modalTitle) => {
    const modal = buildModal(bodyPromise, footerPromise, modalTitle);
    registerListenerEvents(modal, data, searchFunc, !!footerPromise);
};
 ...
 * @param {Array} data An array of all the data generated by the callee.
 * @param {Function} searchFunc Partially applied function we need to manage search the passed dataset.
 * @param {Boolean} hasFooterExpected Given the results of the double bang, if we have a footer do some extra registering.
 */
const registerListenerEvents = (modal, data, searchFunc, hasFooterExpected) => {
    modal.then(modal => {
 ...
 * @param {Promise} footerPromise Footer promise that the caller should resolve.
 * @param {string} modalTitle The name of the search widget.
 * @return {Object} The modal ready to display immediately and render body in later.
 */
const buildModal = (bodyPromise, footerPromise, modalTitle) => {
    return ModalFactory.create({
        type: ModalFactory.types.DEFAULT,
        // TODO: Make this defined by the interface.
        title: modalTitle,
        body: bodyPromise,
        footer: footerPromise ? footerPromise : null,
        small: true,
        scrollable: false,
        templateContext: {
 ...
 * @returns {{bodyPromise: Promise, footerPromiseResolver, bodyPromiseResolver, footerPromise: Promise}}
 */
export const promisesAndResolvers = () => {
    ...
    let footerPromiseResolver;
    const footerPromise = new Promise(resolve => {
        footerPromiseResolver = resolve;
    });
    return {bodyPromiseResolver, bodyPromise, footerPromiseResolver, footerPromise};
};
