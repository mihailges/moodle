        ...
        update_file: function(confirmed) {
            ...
            if (fileinfo.type == 'folder') {
                ...
                if ((filenamechanged || filepathchanged) && !confirmed) {
                    ...
                    var originalfileextension = (originalfilenamearr.length > 1) ? originalfilenamearr.pop() : "";
                    var newfilenamearr = newfilename.split('.');
                    var newfileextension = (newfilenamearr.length > 1) ? newfilenamearr.pop() : "";

                    if (newfileextension !== originalfileextension) {
                        message += '<p>' + M.util.get_string('originalextensionchange', 'repository') + '</p>';
                    }
                    if (fileinfo.refcount) {
                        message += '<p>' + M.util.get_string('confirmrenamefile', 'repository', fileinfo.refcount) + '</p>';
                    }
                    if (message.length > 0) {
