...
class scales_action_bar extends action_bar {
    ...
    /** @var int|null $courseid The course ID if in a course context, otherwise null. */
    protected $courseid;

    /**
     * The class constructor.
     *
     * @param int|null $courseid The course ID if in a course context, otherwise null.
     */
    ...
    public function export_for_template(\renderer_base $output): array {
        ...
        if ($this->courseid) {
            ...
            $generalnavselector = new general_action_bar($this->courseid,
                new moodle_url('/grade/edit/scale/index.php', ['id' => $this->courseid]), 'scale', 'scale');
            $data = $generalnavselector->export_for_template($output);
        }
        // Add a button to the action bar with a link to the 'add new scale' page.
        $addnewscalelink = new moodle_url('/grade/edit/scale/edit.php', ['courseid' => $this->courseid ?: 0]);
