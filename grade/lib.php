 ...
 * @param actionbar $actionbar The actions bar which will be displayed on the page if $shownavigation is set to true.
 *                             If $actionbar is not explicitly defined, the general action bar
 *                             (\core_grades\output\general_action_bar) will be used by default.
 * @return string HTML code or nothing if $return == false
 */
function print_grade_page_head($courseid, $active_type, $active_plugin = null, $heading = false, $return = false,
       $buttons = false, $shownavigation = true, $headerhelpidentifier = null, $headerhelpcomponent = null, $user = null,
    ...
    if ($shownavigation && $courseid !== SITEID) {
        $renderer = $PAGE->get_renderer('core_grades');
        // If the navigation action bar is not explicitly defined, use the general (default) action bar.
        if (!$actionbar) {
            $actionbar = new general_action_bar($courseid, $PAGE->url, $active_type, $active_plugin);
        }
