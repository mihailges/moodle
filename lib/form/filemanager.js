...
M.form_filemanager.init = function(Y, options) {
    ...
    Y.extend(FileManagerHelper, Y.Base, {
        ...
        update_file: function(confirmed) {
            ...
            var extension = Y.Lang.trim(selectnode.one('.fp-saveas input[name="extension"]').get('value'));
            var name = Y.Lang.trim(selectnode.one('.fp-saveas input[name="name"]').get('value'));
            var newfilename = (extension) ? name + "." + extension : name;
        ...
        select_file: function(node) {
            ...
            if (node.hasextension) {
                ...
                extension = (namearr.length > 1) ? namearr.pop() : "";
                name = (namearr.length > 1) ? namearr.join('.') : namearr;
            }
            selectnode.one('.fp-saveas input[name="name"]').set('value', name);
            selectnode.one('.fp-saveas input[name="extension"]').set('value', extension);
